...
filter to enrich for bona fide hyperedited regions
"""
...
def count_edits(alignment_list, read_type, 
        ref, alt, c_ref, c_alt, lib_type = 'fr-first-strand'):
    ...
    lib_type = 'fr-second-strand' inverts this behavior"""
    
    edit_count = 0 
    total_count = 0
    
    if lib_type == 'fr-second-strand':
        sense_read = 'R1'
    elif lib_type == 'fr-first-strand':
        sense_read = 'R2'
    else:
...
def best_multimapper(bam, alignment, fasta_obj, read_type, ref, alt, c_ref,
        c_alt, lib_type = 'fr-first-strand'):
    ...
    if(len(rseq) != len(qseq)):
      sys.stderr.write(f"warning: issue parsing {alignment.query_name}\n:query_seq={qseq}\nref_seq={rseq}\n")
      sys.stderr.write(f'alignment:{alignment.to_string()}\n')
      return None 

    new_align = align_seqs(qseq, rseq, qquals)
    edit_count, total_count, filtered_alignment = count_edits(new_align,
            lib_type, ref, alt, c_ref, c_alt, lib_type)
    ...
    for other_alignment in xa_list:
        ...
        pos = int(position[1:]) - 1 #strip off "+", convert to 0-based
        ...
        mm_r_seq = fasta_obj.fetch(reference=chrom, 
                start = pos, 
                end = pos + align_len)
            
        if strand == "-": 
            mm_r_seq = rev_comp(mm_r_seq)
        
        if(len(mm_r_seq) != len(qseq)):
            # this happens with multimappers that cross two contigs
            # bwa concatenates reference sequences when building the index
            # this is a rare issue
            sys.stderr.write(f"warning: issue parsing {alignment.query_name}\n:query_seq={qseq}\nref_seq={mm_r_seq}\n")
            sys.stderr.write(f'alignment:{alignment.to_string()}\n')
            continue

        new_align = align_seqs(qseq, mm_r_seq, qquals)
   
        edit_count, total_count, filtered_alignment = count_edits(new_align, read_type, ref, alt, c_ref, c_alt, lib_type)
        percent = edit_count / float(q_len)
    ...
    # can happen for rare alignments crossing 2 contigs
    if len(alignment_info) < 2:
        return None
    ...
    if max_ag > (second_ag * 1.10):
        ...
        alignment.reference_id =  bam.get_tid(best_alignment[0])
        alignment.reference_start = best_alignment[1]
        alignment.cigarstring = best_alignment[2]
        return alignment

def process_bam(bam, fasta, output_edits, output_bed, read_type, lib_type, edit_type):
    ...
    for alignment in bamfile_obj.fetch():
        ...
        if alignment.has_tag("XA"): 
            best_alignment = best_multimapper(bamfile_obj, alignment, fasta_obj,
                    read_type, ref, alt, c_ref, c_alt, lib_type)  
            if best_alignment is not None:
                # technically alignment is already modified in place by
                # best_multimapper, but for ease of reading reassign
        ...
        #require at least 5% of sequence is edited or at least bases
        ...
        # report strand based on Read type and library-type 
        if lib_type == 'fr-second-strand':
            antisense_read = 'R2'
        elif lib_type == 'fr-first-strand':
            antisense_read = 'R1'
...
def main():
    ...
    parser.add_argument('-s',
                        ...
                        help ="""R1 or R2 from paired end alignment, 
                          ...
                          for R2 in PE fr-first-strand orientation, an
                          editing site on the
    ...
    parser.add_argument('-l',
                        '--library-type',
                        help = """ Library orientation, either:
                        fr-first-strand or fr-second-strand.
                        For fr-second-strand, R1 is sense and R2
                        antisense. Defaults to more common fr-first-strand
                        """,
                        required = False,
                        default = "fr-first-strand")
    ...
    process_bam(bam_name, fa_name, edits, reads, strandedness, lib_type, edit_type)
